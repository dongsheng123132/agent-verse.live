...
export const Navigation: React.FC<NavigationProps> = ({ currentSection, setSection }) => {
  ...
  const getActiveSection = () => {
    if (pathname === '/chunwan' || pathname === '/points' || pathname === '/predictions') return NavSection.GALA;
    if (pathname === '/forum') return NavSection.FORUM;
    if (pathname === '/market') return NavSection.MARKET;
    if (pathname === '/events') return NavSection.EVENTS;
    if (pathname === '/ai-insights') return NavSection.AI_CONTENT;
    if (pathname === '/map') return NavSection.MAP;
    if (pathname === '/dashboard' || pathname === '/') return NavSection.HOME;
  ...
  const navItems = [
    { id: NavSection.HOME, label: 'Dashboard', icon: LayoutGrid, href: '/dashboard' },
    { id: NavSection.MAP, label: 'World Map', icon: Map, href: '/map' },
    { id: NavSection.EVENTS, label: 'Events', icon: Calendar, href: '/events' },
    { id: NavSection.MARKET, label: 'Market', icon: ShoppingBag, href: '/market' },
    { id: NavSection.FORUM, label: 'Forum', icon: MessageSquare, href: '/forum' },
    { id: NavSection.AI_CONTENT, label: 'AI Insights', icon: Newspaper, href: '/ai-insights' },
    ...
    <nav className="fixed top-0 left-0 right-0 h-16 glass-panel border-b border-gray-800 z-50 flex items-center justify-between px-6">
      ...
      <div className="hidden md:flex gap-1">
        {navItems.map((item) => {
          ...
          const activeClass = item.id === NavSection.GALA && isActive
      ...
      <div className="flex items-center gap-4">
        <Link 
          ...
          className={`hidden sm:flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-900 to-indigo-900 border border-purple-500/50 rounded-lg text-xs font-bold uppercase tracking-wider hover:brightness-110 transition-all ${
            pathname === '/connect' ? 'ring-1 ring-claw-accent' : ''
          }`}
        >
          <Wallet size={14} />
          Connect
